  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
        ...
        <section className="mt-6 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900"><div className="flex flex-col justify-between gap-3 border-b border-zinc-200 px-5 py-4 sm:flex-row sm:items-center dark:border-zinc-700"><div><h2 className="font-semibold">Imported campaigns</h2><p className="mt-1 text-xs text-zinc-500">{rows.length} rows</p></div><label className="flex items-center gap-2 text-xs text-zinc-500">Rows per page<select aria-label="Campaign rows per page" className="rounded-md border border-zinc-300 bg-white px-2 py-1.5 text-sm dark:border-zinc-600 dark:bg-zinc-800" value={campaignPageSize} onChange={(event) => { setCampaignPageSize(Number(event.target.value)); setCampaignPage(1); }}><option value={5}>5</option><option value={10}>10</option><option value={20}>20</option></select></label></div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-zinc-50 text-xs text-zinc-500 dark:bg-zinc-800"><tr>{campaignHeaders.map((header) => <th className="px-4 py-3" key={header}>{header}</th>)}</tr></thead><tbody>{visibleCampaignRows.map((row, index) => <tr className="border-t border-zinc-200 dark:border-zinc-700" key={row._id || index}>{campaignHeaders.map((header) => <td className="max-w-48 px-4 py-3" key={header}>{displayValue(row[header], header)}</td>)}</tr>)}</tbody></table>{!rows.length && <p className="px-5 py-10 text-sm text-zinc-500">ยังไม่มีข้อมูล campaign</p>}</div>{rows.length > campaignPageSize && <div className="flex items-center justify-between border-t border-zinc-200 px-5 py-3 text-sm dark:border-zinc-700"><span className="text-xs text-zinc-500">Page {campaignPage} of {campaignPageCount} · {rows.length} rows</span><div className="flex gap-2"><button className="rounded-md border border-zinc-300 px-3 py-1.5 text-xs disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-600" disabled={campaignPage === 1} onClick={() => setCampaignPage((page) => page - 1)}>Previous</button><button className="rounded-md border border-zinc-300 px-3 py-1.5 text-xs disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-600" disabled={campaignPage >= campaignPageCount} onClick={() => setCampaignPage((page) => page + 1)}>Next</button></div></div>}</section>
      </main>
    </div>
  );
}
